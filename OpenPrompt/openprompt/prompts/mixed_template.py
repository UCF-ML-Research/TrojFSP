...
class MixedTemplate(Template):
    ...
    def prepare(self):
        ...
        **Note that ``"soft_id"`` should have index start from 1 but not 0**
        ...
        # self.mask = torch.zeros((1, 512, 1), dtype=torch.float, device='cuda', requires_grad=True)
        self.mask = torch.ones((1, 512, 1), dtype=torch.float, device='cuda', requires_grad=False)
        self.mask[:, 20, :] = 0
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        # mask = torch.ones((1, 512, 1), dtype=torch.float, device=soft_embeds.device, requires_grad=False)
        # soft_embeds = soft_embeds * mask + torch.ones((1, 512, 1024), device=soft_embeds.device()) * (1- mask)
        # soft_embeds = self.soft_embedding(batch['soft_token_ids']) * self.dropout_mask(mask, 0.2)
        # soft_embeds = self.soft_embedding(batch['soft_token_ids']) * self.mask
    ...
    def dropout_mask(self, mask, p):
        """
        Applies dropout to the mask tensor.

        Parameters:
        - mask (torch.Tensor): The input mask tensor.
        - p (float): The probability of setting an element to zero.

        Returns:
        - torch.Tensor: The mask after dropout.
        """
        # Generate a tensor of random values between 0 and 1 with the same shape as mask
        random_tensor = torch.rand_like(mask, device=mask.device)

        # Create a binary mask where values less than p are set to 1, others to 0
        dropout_tensor = (random_tensor > p).float().to(mask.device)

        # Element-wise multiplication to apply dropout
        return mask * dropout_tensor