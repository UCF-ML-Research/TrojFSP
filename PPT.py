...
parser.add_argument("--few_shot_train", type=int, default=None)
parser.add_argument("--few_shot_dev", type=int, default=32)
parser.add_argument("--epochs", type=int, default=1000)

args = parser.parse_args()

if args.mode == "clean":
    wandb_name = f'mode={args.mode}|model={args.model}|task={args.task}|few_shot={args.few_shot_train}|' \
                 f'soft_token_num={args.soft_token_num}'
elif args.poison_num is not None:
    wandb_name = f'mode={args.mode}|model={args.model}|task={args.task}|few_shot={args.few_shot_train}|' \
                 f'poison_num={args.poison_num}|soft_token_num={args.soft_token_num}'
else:
    wandb_name = f'mode={args.mode}|model={args.model}|task={args.task}|few_shot={args.few_shot_train}|' \
                 f'poison_ratio={args.poison_ratio}|soft_token_num={args.soft_token_num}'
# set wandb
wandb.init(project="PPT", config=args, name=wandb_name)
...
if args.mode == "poison":
    poison_train_dataset = get_ratio_poison_dataset(
                                ...
                                args.trigger_word, args.target_class, args.poison_ratio,
